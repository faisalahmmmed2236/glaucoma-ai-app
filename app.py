...
def generate_gradcam(model, img_array, layer_name):
    ...
    with tf.GradientTape() as tape:
        ...
        # FIX: Updated to index 0 for the single-node binary model
        loss = predictions[:, 0] 
...
with st.sidebar:
    ...
    threshold = st.slider("Diagnostic Threshold", min_value=0.10, max_value=0.90, value=0.50, step=0.01, 
...
if uploaded_file is not None:
    ...
    with st.spinner('Processing retinal image...'):
        ...
        # FIX: Extracting the single probability value from the binary output
        glaucoma_prob = float(prediction[0])
